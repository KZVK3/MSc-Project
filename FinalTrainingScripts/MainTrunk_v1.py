...
import os, json
from modules import EmbeddingsAndEvoformer, makeLinear#, TriangleMultiplication, TriangleAttention, Transition

import matplotlib.pylab as plt
from IPython import display

def dyplot(fig, axs, ims, hdisplay, pred, true, lr):
  ims[0].set_data(pred.cpu())
  axs[0].set_title('lr: %s, min: %.2f, max: %.2f'%(str(lr), pred.min(), pred.max()))

  ims[1].set_data(true.cpu())
  axs[1].set_title('min: %.2f, max: %.2f'%(true.min(), true.max()))
...
class BaseTrunk(pl.LightningModule):
  ...
  def forward(self, batch):
    ...
    logits = self.distogram_head( representations )
    out = self.distogram_head.loss(logits, batch)
    return out
  ...
  def training_step(self, train_batch, batch_idx):
    ...
    if batch_idx%self.plot_every==0:
      dargs = (out['pred'].data, out['target'].data, self.lr_schedulers().get_last_lr())
      dyplot(*self.plot_args, *dargs)

    if self.time > self.wait_time and code in self.t_track:
      self.save_data('train_disto', out['pred'].data.cpu().numpy(), False, code)
      self.time = 0
    self.time += 1
  ...
  def validation_step(self, val_batch, batch_idx):
    ...
    if code in self.v_track:
      self.save_data('val_disto', out['pred'].data.cpu().numpy(), False, code)
